    ...
    : QWidget(parent)
{
    ...
    xAxis->init(XYAxis::AtBottom, 0, 1800, 100, 0, 1740);
    yAxis = new XYAxis(this);
    yAxis->init(XYAxis::AtLeft, -0.5, 1.2, 0.001, -0.4, 1);
//    topAxis = new XYAxis(this);
//    topAxis->init(XYAxis::AtTop, 0, 1800, 100, 0, 1740);
//    rightAxis = new XYAxis(this);
//    rightAxis->init(XYAxis::AtTop, -0, 1.2, 0.001, -0.4, 1);

    connect(xAxis, &XYAxis::axisChanged, this, &XYView::refresh);
    connect(yAxis, &XYAxis::axisChanged, this, &XYView::refresh);
//    connect(topAxis, &XYAxis::axisChanged, this, &XYView::refresh);
//    connect(rightAxis, &XYAxis::axisChanged, this, &XYView::refresh);
...
//    painter.drawRect(plotArea.adjusted(0, 1, 1, 1));
...
void XYView::setdata(QVector<pair<float, float>> qlqpfTemp)
{
    for(const pair<float, float> &item : qlqpfTemp)
        seriesData.append(Node{item.first, item.second});
    update();
}
